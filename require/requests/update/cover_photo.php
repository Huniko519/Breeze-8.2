...
if((isset($_SESSION['username']) && isset($_SESSION['password'])) || (isset($_COOKIE['username']) && isset($_COOKIE['password']))) {
	...
	if(!empty($user['idu'])) {	
		...
		if($image_size_info && $image_size <= $page_settings['max_img_size']*1000000){
			...
			if(function_exists('exif_read_data') && $image_res) {
			...
			if($image_res){
				...
				$image_save_folder_rep 	= $page_settings['folder_cover_photos'] .'rep_'. $new_file_name;	
				...
				if(sizeImage($image_res, $image_save_folder, $image_type, $page_settings['max_cover_pics'], $image_width, $image_height, $page_settings['jpeg_quality'] )){	            
					...
					// Add repositioned cover file
					copy($image_save_folder, $image_save_folder_rep);
				...
				// Invalid format
				$return = ($bad) ? showError($TEXT['_uni-upl_hgh_q_t_cnt']) : showError($TEXT['_uni-Photo-invalid']);
			}		
		} else {
...
<script language="javascript" type="text/javascript">
	window.top.window.loadImageFull('#profile_view_cover_<?php echo $user['idu'] ; ?>','<?php echo $TEXT['installation'];?>/uploads/profile/covers/<?php echo $updated; ?>',<?php echo $user['idu'] ; ?>,1);
</script>
<?php } else { ?>
<script language="javascript" type="text/javascript">
