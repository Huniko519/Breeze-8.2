	...
	if(!empty($user['idu']) && $joined) {	
		...
		if($image_size_info && $image_size <= $page_settings['max_img_size']*1000000){
			...
			if($image_res){
                ...
                $image_save_folder_rep 	= $page_settings['folder_group_cover_photos'] .'rep_'. $new_file_name;				
...
<script language="javascript" type="text/javascript">
	window.top.window.loadImageFull('#group_view_cover_<?php echo $_POST['group-cover-form-id'] ; ?>','<?php echo $TEXT['installation'];?>/uploads/groups/<?php echo $updated; ?>',0,1);
</script>
<?php } else { ?>
<script language="javascript" type="text/javascript">
