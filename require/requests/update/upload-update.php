			...
			if(move_uploaded_file($zip_temp, $target_zip)) {
        		...
        		if ($opened_zip === true) {
					...
					if(!$checked) {
						...
						// Go to update classes.php file
						if(file_exists('../../../updates/'.$name[0].'/KTA.express.php')) {
							$link = $TEXT['installation'].'/updates/'.$name[0].'/KTA.express.php';
						} else {
							$return = "1";
						}
...
<script language="javascript" type="text/javascript">
	window.top.window.locate('<?php echo $db->real_escape_string($link); ?>');
</script>
<?php } elseif(isset($return)) {
	?>
<script language="javascript" type="text/javascript">
